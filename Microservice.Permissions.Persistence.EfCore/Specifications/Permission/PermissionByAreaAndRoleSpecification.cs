...
{
    public sealed class PermissionByAreaAndRoleSpecification : Specification<PermissionEntity>
    {
        private readonly int areaId;
        private readonly int roleId;
        private readonly string[] permissions;

        public PermissionByAreaAndRoleSpecification(int areaId, int roleId, params string[] permissions)
        {
            this.areaId = areaId;
            this.roleId = roleId;
            ...
            IQueryable<PermissionEntity> query)
        {
            var result = query.Where(x => x.PermissionCollection.AreaId == areaId
                                          && x.PermissionCollection.RoleId == roleId
                                          && permissions.Any(y => x.Name.ToUpper() == y));
            return result;
        }
    }
}